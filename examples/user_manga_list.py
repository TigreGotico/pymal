...
username = "Xinil"
entries = pymal.get_user_manga_list(username)

print(f"Manga list for {username}: {len(entries)} entries")
print()
print(f"{'Title':<45} {'Status':<15} {'Score':<7} {'Chapters'}")
print("-" * 85)
for entry in entries[:20]:
    score = str(entry.score) if entry.score else "—"
    total = str(entry.total_chapters) if entry.total_chapters else "?"
    progress = f"{entry.chapters_read}/{total}"
